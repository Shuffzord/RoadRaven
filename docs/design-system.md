...
- **Autosave.** Every accepted change is written to the theme file 500 ms after the last one (`Saved · just now` / `Saving…` / `Could not save: …`). **Revert to saved** puts the draft back to the last successful write. Closing flushes a pending change first and only asks (`Keep editing` / `Discard changes`) if that write failed. `Escape` closes the editor only while focus is inside it; closing reopens Preferences with focus on Edit….
