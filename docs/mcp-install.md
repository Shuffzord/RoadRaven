...
> `app_not_running`.
>
> Every command below pins the exact version, `0.8.0`. Pin the version
...
Follow the prompts and give it the command `npx -y @roadraven/mcp@0.8.0`.
...
```json
{
  "mcp": {
    "roadraven": {
      "type": "local",
      "command": ["npx", "-y", "@roadraven/mcp@0.8.0"],
      "enabled": true
    }
  }
}
